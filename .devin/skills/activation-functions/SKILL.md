# Skill: Activation Functions — Theory, Math & Implementation

## When to invoke this skill
Invoke when working on any of the following in mlp-study-kit:
- `nn_core/activations.py` — canonical ActivationFn class
...
No matter how many layers you stack, the result is still **linear** — equivalent to a
...
## 3. Activation Functions — Complete Reference
...
- No vanishing gradient — gradient stays exactly 1
...
- ex07–ex10 hidden layers when testing basic propagation
...
causes gradients to shrink exponentially toward the first layers → weights near the input
barely update → network trains very slowly.

**Numerical stability:**
- For very negative v: `exp(-v)` overflows. Use: `1 / (1 + exp(-v))` directly (NumPy handles this correctly with float64)
- For very positive v: `exp(-v)` underflows to 0, giving `f(v) ≈ 1` correctly
...
- Zero-centered: output mean ≈ 0 → helps next layer (no bias shift)
- Derivative peaks at v=0: f'(0) = 1.0

**Tanh vs Sigmoid:**
- Tanh output range (-1, 1) vs sigmoid (0, 1)
- Tanh is zero-centered — preferred for hidden layers over sigmoid
...
**Code (nn_core/activations.py — numerically stable via np.tanh):**
```python
@staticmethod
...
Note: f'(0) is undefined mathematically, but set to 1 (or 0) by convention — doesn't matter in practice.
...
3. Sparse activation: ~50% of neurons output 0 in a random init → efficient
4. Faster convergence in practice

**Dying ReLU problem:**
If a neuron's weights are updated such that its `v` is always negative, its gradient
is permanently 0 → the neuron never recovers ("dies"). Typically 10–30% of ReLU
...
- Mean activations closer to 0 → faster convergence
...
| Output — binary classification | Sigmoid | Output in (0,1) |
| Output — multi-class | Softmax* | Probabilities sum to 1 |

*Softmax is not implemented in this codebase — add it to `nn_core/activations.py` if needed.
...
many layers — early layers stop learning. Sigmoid and Tanh are worst for this.
...
| OOP ex07–ex10 implementations | `exercises/ex07_forward_pass.py` … | `Activation_fcn` class |
