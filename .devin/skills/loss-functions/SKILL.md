# Skill: Loss Functions — Theory, Math & Implementation

## When to invoke this skill
Invoke when working on:
- `nn_core/losses.py` — canonical LossFn class
...
The `0.5` factor is a mathematical convenience — when we differentiate, the 2 from
...
When y > t (overestimate), gradient is positive → weight update moves y down.
When y < t (underestimate), gradient is negative → weight update moves y up.
...
rmse_val = rmse(y_true, y_pred)   # Root MSE — same units as target
mae_val  = mae(y_true, y_pred)    # Mean Absolute Error — outlier robust
...
convenience — the 0.5 only matters for the derivative in backprop, not for evaluation.
...
For binary classification (two classes: 0 and 1), with target `t ∈ {0, 1}` and
network output `y ∈ (0, 1)` (from sigmoid):

```
E = -[ t * log(y) + (1-t) * log(1-y) ]
```

When `t = 1`: `E = -log(y)` — loss is 0 when y=1, infinity when y=0
When `t = 0`: `E = -log(1-y)` — loss is 0 when y=0, infinity when y=1
...
- Gradient near saturated regions (y≈0 or y≈1) is very small because sigmoid derivative ≈ 0
- → Very slow learning when predictions are confident but wrong

**BCE with sigmoid output:**
- Gradient = y - t regardless of saturation
- → Large gradient when wrong (y=0.01, t=1 → gradient = -0.99), even in saturated regions
- → Much faster convergence

### 3.5 Numerical Stability

The formula `log(y)` is undefined for `y=0` and returns -inf for `y→0`. Add epsilon:

```python
# This is NOT in the current implementation — add if needed for stability:
eps = 1e-12
...
Forward pass:
  v = W @ input + bias
  y = f(v)              ← activation function
  E = loss(t, y)        ← loss function (scalar per sample)

Backward pass:
  delta_output = dE/dy * f'(v)   ← output layer delta
  delta_hidden = (W_next.T @ delta_next) * f'(v)   ← hidden layer deltas
...
It is the ONLY contribution of the loss function to backpropagation — everything else
comes from the activation derivatives.

### Code (nn_core/network.py — backward_propagate):
```python
