# Skill: Neural Network Training — Weight Init, Optimizers & Practical Guide
...
- `modules/data_utils.py` — normalize, train_test_split, k_fold_split
...
2. Architecture design
   └─ input units → hidden layers → output units + activations

3. Weight initialization
   └─ random, Glorot Normal, or fixed
...
6. Persist
   └─ save_weights() → outputs/model.npz
```
...
# No pandas required — stdlib csv module only
```
...
Always split BEFORE normalizing — compute stats on train set only.
...
```
After: mean ≈ 0, std ≈ 1 per feature
...
the origin — with bias, it can shift freely.
...
All-zeros: every neuron computes the same gradient → symmetric weights → network
fails to learn (symmetry breaking problem).

All-large: activations saturate (sigmoid/tanh) → vanishing gradient.
...
### 4.3 Glorot Normal (Xavier Normal) — used in experiments/20251224_v2.py
...
```
Var(w) = 2/n_in   →  sigma = sqrt(2/n_in)
```

**Best for:** ReLU and its variants.
**Not in nn_core yet** — add if training deep ReLU networks.

---

## 5. Optimizers

### 5.1 SGD — Stochastic Gradient Descent (in nn_core/network.py)
...
**Online SGD** (one update per sample — what nn_core does):
- High variance in updates, but fast convergence in early epochs
- Good for noisy problems (noise helps escape local minima)

**Mini-batch SGD** (not in nn_core — could add by accumulating gradients):
- Balance between online and batch
- Better GPU utilization

### 5.2 Adagrad — Adaptive Gradient (in experiments/20251224_v2.py)
...
**Advantage:** Automatically adjusts lr per parameter — useful for sparse features.
**Disadvantage:** G_w grows monotonically → effective lr eventually → 0, training stops.

### 5.3 Adam (NOT in this codebase — would add as follows)
...
Stops training when the model stops improving on the validation set — prevents overfitting.
...
Start with 1.0 — if gradients are well-behaved, it rarely triggers.
...
print(f"R²:   {r2_score(Y_te_flat, preds):.4f}")
