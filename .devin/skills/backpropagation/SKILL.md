# Skill: Backpropagation — Complete Theory & Implementation

## When to invoke this skill
Invoke when working on:
- `nn_core/network.py` — `backward_propagate()`, `update_weights()`, `train()`
...
If `dE/dw_ji > 0`: increasing w_ji increases E → decrease w_ji
If `dE/dw_ji < 0`: increasing w_ji decreases E → increase w_ji
If `dE/dw_ji = 0`: w_ji is at a local minimum → leave it alone

**Weight update rule (Gradient Descent / SGD):**
```
w_ji(new) = w_ji(old) - eta * dE/dw_ji
```
Where `eta` (η) is the learning rate — controls step size.
...
```
x → [W1] → v1 → [f1] → y1 → [W2] → v2 → [f2] → y2 → [Loss] → E
```
...
**Code (nn_core/network.py — forward_propagate):**
```python
...
## 6. Backward Pass — Delta Computation

The **delta** δ^(l) for layer l is defined as:
```
...
**Special case — sigmoid output with BCE loss:**
```
delta^(L) = y^(L) - t   (simplified — see loss-functions skill)
```
...
```
W^(l)(new) = W^(l)(old) - eta * delta^(l) ⊗ y^(l-1)
```

Where `⊗` is the **outer product** — produces a matrix of shape (n_l, n_{l-1}):
...
**Code (nn_core/network.py — _update_weights_clipped):**
```python
...
for x_row, y_row in zip(x_train, y_train):
    # Step 1: Forward pass — compute all v^(l) and y^(l)
    self.forward_propagate(nnetwork, x_row)

    # Step 2: Compute deltas — backward pass
...
This is **online (stochastic) gradient descent** — one weight update per sample.
...
### Loss not decreasing
```
Cause A:  Learning rate too small → try 10x larger
Cause B:  Learning rate too large → loss oscillates or increases → try 10x smaller
Cause C:  Wrong activation for output layer (e.g., relu for BCE → negative outputs)
Cause D:  Dying ReLU → switch to Leaky ReLU
Cause E:  Not enough epochs
Cause F:  Poor weight initialisation → check make_regression_data returns sane values
```
