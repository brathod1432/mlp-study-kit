...
def demo_task4_regression_ex10(
    ...
    Task 4 Demonstration:
      - If use_hw1_compute_cost=False: Exercise 10 regression dataset
      - If use_hw1_compute_cost=True : HW1 compute_cost-based regression dataset
    """
    title_message("Task4\tRegression Demo", color="magenta")

    if use_hw1_compute_cost:
        X, T = make_regression_data_from_hw1_compute_cost(
            n_samples=300,
    ...
    layers_base = [
        init_layer_fixedstd_ex10(in_dim, 10, fcn="relu", std=0.5, seed=10),
        init_layer_fixedstd_ex10(10, 1, fcn="linear", std=0.5, seed=20),
    ]
    loss_base = train_mlp_ex10(
        X_train, T_train, layers_base,
        epochs=epochs, lr=lr,
        update_fn=nn_backward_update_sgd_ex10,
    ...
    layers_imp = [
        init_layer(in_dim, 10, fcn="elu", alpha=1.0, seed=30),
        init_layer(10, 1, fcn="linear", alpha=None, seed=40),
    ]
    loss_imp = train_mlp_ex10(
        X_train, T_train, layers_imp,
        epochs=epochs, lr=lr,
        update_fn=nn_backward_update,
        eps=1e-8,
        log_every=max(1, epochs // 10),
    )
...
def make_regression_data_from_hw1_compute_cost(
    ...
    if food_items is None:
        food_items = ["banana", "orange", "apple"]  # default items
    ...
    for i in range(n_samples):
        stock = {item: int(rng.integers(0, max_stock + 1)) for item in food_items}  # random stock
        prices = {item: float(rng.uniform(price_low, price_high)) for item in food_items}  # random prices

        stocks_vec = np.array([stock[item] for item in food_items], dtype=np.float64)  # stocks
        prices_vec = np.array([prices[item] for item in food_items], dtype=np.float64)  # prices

        X[i, :k] = stocks_vec  # first half stock
        X[i, k:] = prices_vec  # second half price

        total = compute_cost(food=food_items, stock=stock, prices=prices, raiseException=True)  # target
        T[i, 0] = float(total)
